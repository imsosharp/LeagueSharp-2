...
namespace DevKogMaw
{
    class Program
    {
        ...
        public static void Combo()
        {
            ...
            if (!HasWBuff() && Player.AttackRange < Player.Distance(eTarget) && Player.Distance(eTarget) <= (LXOrbwalker.GetAutoAttackRange(eTarget) + W.Range) && W.IsReady() && useW)
            {
                if (packetCast)
        ...
        private static void JungleSteal()
        {
            ...
            var query = mobs.Where(x =>
                ...
                DevHelper.GetEnemyList().Any(enemy => x.Distance(enemy.ServerPosition) < 1000)).ToList();
